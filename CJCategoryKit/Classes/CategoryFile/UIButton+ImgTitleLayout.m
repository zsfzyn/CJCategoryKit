...
//定义关联的Key
static const char * titleRectKey = "cjzsf_titleRectKey";
- (CGRect)titleRect {
...
//定义关联的Key
static const char * imageRectKey = "cjzsf_imageRectKey";
- (CGRect)imageRect {
...
+ (void)load {
    ...
    MethodSwizzle(self,@selector(titleRectForContentRect:),@selector(cj_override_titleRectForContentRect:));
    MethodSwizzle(self,@selector(imageRectForContentRect:),@selector(cj_override_imageRectForContentRect:));
}
...
- (CGRect)cj_override_titleRectForContentRect:(CGRect)contentRect {
    
    if (!CGRectIsEmpty(self.titleRect) && !CGRectEqualToRect(self.titleRect, CGRectZero)) {
        return self.titleRect;
    }
    return [self cj_override_titleRectForContentRect:contentRect];
    
}

- (CGRect)cj_override_imageRectForContentRect:(CGRect)contentRect {
    
    if (!CGRectIsEmpty(self.imageRect) && !CGRectEqualToRect(self.imageRect, CGRectZero)) {
        return self.imageRect;
    }
    return [self cj_override_imageRectForContentRect:contentRect];
}
